...
public:
    ...
    static constexpr uint64_t file_size = 10ul << 30;
...
void write_configuration_file(unsigned max_io_requests, std::experimental::optional<unsigned> num_io_queues = {}) {
    std::string conf_file = "/.config/seastar/seastar-io.conf";
    ...
    if (home) {
        try {
            ...
            ::mkdir(".config/seastar", S_IRWXU);

            std::ofstream ofs_io;
            ofs_io.exceptions(std::ofstream::failbit | std::ofstream::badbit);
            ofs_io.open(std::string(home) + conf_file, std::ofstream::trunc);
            if (ofs_io) {
            ...
            ofs_io.close();
            std::cout << "Written the above values to " << std::string(home) << conf_file << std::endl;
        } catch (std::ios_base::failure& e) {
            std::cout << e.what() << " when writing configuration file. Please add them to your seastar command line" << std::endl;
        }
    } else {
        std::cout << "$HOME not set, so not writing the above values. Please add them to your seastar command line" << std::endl;
    }
}
...
int main(int ac, char** av) {
    ...
    if (num_io_queues != cpuvec.size()) {
        ...
        write_configuration_file(iodepth, num_io_queues);
    } else {
        write_configuration_file(iodepth);
    }
    return 0;
}
