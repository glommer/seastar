...
bool sanity_check_ev(const io_event& ev, size_t size, std::unordered_set<long> accepted_errors = {}) {
    if (long(ev.res) < 0) {
        if (accepted_errors.count(-long(ev.res))) {
            return false;
        }
...
void test_file::generate(iotune_manager& iotune_manager, std::chrono::seconds timeout) {
    ...
    while (pos < iotune_manager.file_size || aio_outstanding) {
        ...
        if (aio_outstanding) {
            ...
            throw_kernel_error(n);
            for (auto i = 0ul; i < size_t(n); ++i) {
                if (sanity_check_ev(ev[i], iotune_manager::wbuffer_size, { ENOSPC })) {
