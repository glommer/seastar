...
struct test_file {
    ...
    void generate(std::chrono::seconds timeout);
...
public:
    ...
    {
        _test_file.generate((timeout * 4) / 10);
...
void test_file::generate(std::chrono::seconds timeout) {
    ...
    while (pos < iotune_manager::file_size || aio_outstanding) {
        ...
        if ((latest_tstamp - start_time) > timeout) {
            std::cerr << "timed out before finishing writing test file. Wrote only " << test_file::to_gb(bytes_written) << "GB." << std::endl;
            ...
            std::cerr << "Will continue anyway, but accuracy will suffer" << std::endl;
            break;
        }

    }
    iotune_manager::file_size = bytes_written;
    std::cout << test_file::to_gb(iotune_manager::file_size) << "GB written in "
