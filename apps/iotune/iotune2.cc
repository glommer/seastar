...
public:
    ...
    uint64_t maximum_io_size() const {
        ...
        return std::min(_max_data_transfer_size, uint64_t(2) << 20);
    }
...
public:
    ...
    future<io_rates> write_sequential_data(unsigned shard, size_t buffer_size) {
        return _iotune_test_file.invoke_on(shard, [buffer_size] (test_file& tf) {
            auto duration = std::chrono::duration_cast<std::chrono::duration<double>>(30s) / smp::count;
            return tf.write_workload(buffer_size, test_file::pattern::sequential, 8, duration);
        });
    }
    ...
    future<io_rates> read_sequential_data(unsigned shard, size_t buffer_size) {
        return _iotune_test_file.invoke_on(shard, [buffer_size] (test_file& tf) {
            ...
            return tf.read_workload(buffer_size, test_file::pattern::sequential, 8, duration);
        });
    }
