...
struct fair_queue_request_descriptor {
    ...
    unsigned iops_weight = 1; ///< the weight of this request for capacity purposes (IOPS).
...
public:
    ...
    void notify_requests_finished(fair_queue_request_descriptor& desc) {
        _requests_executing--;
        _req_count_executing -= desc.iops_weight;
    ...
    void dispatch_requests() {
        while (can_dispatch()) {
            ...
            h->_queue.pop_front();
            _requests_executing++;
            _req_count_executing += req.desc.iops_weight;
