...
/// objects, which will each have a \ref shares attribute.
///
...
class fair_queue {
    ...
    void execute_one() {
        _sem.wait().then([this] {
            ...
            h->_accumulated += (exp(1.0/_tau.count() * delta.count()) * req.weight) / h->_shares;
    ...
    void reset_stats() {
        ...
        auto delta = std::chrono::duration_cast<std::chrono::microseconds>(now - _base);
        _base = now;
        for (auto& pc: _all_classes) {
            pc->_accumulated /= (std::numeric_limits<float>::max() / _capacity);
        }
    }
...
public:
    ...
    explicit fair_queue(unsigned capacity,
                        std::chrono::microseconds tau = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::milliseconds(100)))
                                           : _sem(capacity)
                                           , _capacity(capacity)
                                           , _base(std::chrono::steady_clock::now())
                                           , _tau(tau)
                                           , _normalize_timer([this] { reset_stats(); }) {
        _normalize_timer.arm_periodic(unsigned(std::log(std::numeric_limits<float>::max() / _capacity)) * _tau);
    }
    ~fair_queue() {
        _normalize_timer.cancel();
    }
    ...
    /// \throw std::invalid_argument, if trying to unregister a class with pending requests.
    void unregister_priority_class(priority_class_ptr pclass) {
        if (!pclass->_queue.empty()) {
            throw std::invalid_argument("Trying to remove priority class with pending operations");
        }
    ...
    std::result_of_t<Func()> queue(priority_class_ptr pc, unsigned weight, Func func) {
        ...
        return fut.then([func = std::move(func)] {
            return futurize<std::result_of_t<Func()>>::apply(func);
        }).finally([this] {
            _sem.signal();
        });
    }
    ...
    void update_shares(priority_class_ptr pc, uint32_t new_shares) {
