...
public:
    ...
    virtual future<size_t> write_dma(uint64_t pos, const void* buffer, size_t len, unsigned pc) = 0;
    virtual future<size_t> write_dma(uint64_t pos, std::vector<iovec> iov, unsigned pc) = 0;
    virtual future<size_t> read_dma(uint64_t pos, void* buffer, size_t len, unsigned pc) = 0;
    virtual future<size_t> read_dma(uint64_t pos, std::vector<iovec> iov, unsigned pc) = 0;
...
public:
    ...
    future<size_t> write_dma(uint64_t pos, const void* buffer, size_t len, unsigned pc);
    future<size_t> write_dma(uint64_t pos, std::vector<iovec> iov, unsigned pc);
    future<size_t> read_dma(uint64_t pos, void* buffer, size_t len, unsigned pc);
    future<size_t> read_dma(uint64_t pos, std::vector<iovec> iov, unsigned pc);
    future<> flush(void);
...
public:
    ...
    future<size_t>
    dma_read(uint64_t aligned_pos, CharType* aligned_buffer, size_t aligned_len, unsigned pc = default_priority_class()) {
     ...
     */
    template <typename CharType>
    future<temporary_buffer<CharType>> dma_read(uint64_t pos, size_t len, unsigned pc = default_priority_class()) {
    ...
    dma_read_exactly(uint64_t pos, size_t len, unsigned pc = default_priority_class()) {
    ...
    future<size_t> dma_read(uint64_t pos, std::vector<iovec> iov, unsigned pc = default_priority_class()) {
    ...
    template <typename CharType>
    future<size_t> dma_write(uint64_t pos, const CharType* buffer, size_t len, unsigned pc = default_priority_class()) {
    ...
    /// \return a future representing the number of bytes actually written.  A short
    ///         write may happen due to an I/O error.
    future<size_t> dma_write(uint64_t pos, std::vector<iovec> iov, unsigned pc = default_priority_class()) {
    ...
    template <typename CharType>
    future<temporary_buffer<CharType>>
    dma_read_bulk(uint64_t offset, size_t range_size, unsigned pc = default_priority_class());
...
private:
    ...
    template <typename CharType>
    future<temporary_buffer<CharType>>
    read_maybe_eof(uint64_t pos, size_t len, unsigned pc = default_priority_class());
...
template <typename CharType>
future<temporary_buffer<CharType>>
file::dma_read_bulk(uint64_t offset, size_t range_size, unsigned pc) {
...
template <typename CharType>
future<temporary_buffer<CharType>>
file::read_maybe_eof(uint64_t pos, size_t len, unsigned pc) {
    //
