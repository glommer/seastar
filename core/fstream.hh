...
struct file_input_stream_options {
    ...
    priority_class *io_priority_class = &default_priority_class();
};
...
input_stream<char> make_file_input_stream(
        file file, uint64_t offset,
        uint64_t buffer_size = file_input_stream_options().buffer_size) {
    return make_file_input_stream(default_priority_class(), file, offset, buffer_size);
}
...
output_stream<char> make_file_output_stream(
        file file,
        uint64_t buffer_size = 8192) {
    return make_file_output_stream(default_priority_class(), file, buffer_size);
}
