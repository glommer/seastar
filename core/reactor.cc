...
fair_queue::config io_queue::make_fair_queue_config(config iocfg) {
    ...
    // FIXME: many I/O queues together can yield more than max_aio requests in flight
    // we don't want to bring the semaphore back because we want to keep dispatching at
    // every task quota, so maybe dynamically tune this.
...
int reactor::run() {
    ...
    std::vector<std::unique_ptr<io_queue>> clear = {};
    my_io_queues.swap(clear);
    return _return;
}
...
public:
    ...
    std::unordered_map<uint64_t, mountpoint_params> _mountpoints;
...
public:
    ...
    struct io_queue::config generate_config(uint64_t devid) const {
...
void smp::configure(boost::program_options::variables_map configuration)
{
    ...
    std::unordered_map<uint64_t, std::vector<io_queue*>> all_io_queues;
