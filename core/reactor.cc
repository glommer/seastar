...
struct io_desc {
    ...
    io_desc(unsigned fairness_weight, unsigned iops_weight, unsigned size) : fq_desc(fair_queue_request_descriptor{fairness_weight, iops_weight, size}) {}
};
...
io_queue::queue_request(shard_id coordinator, const io_priority_class& pc, size_t len, io_queue::request_type req_type, Func prepare_io) {
    ...
    return smp::submit_to(coordinator, [start, &pc, len, req_type, prepare_io = std::move(prepare_io), owner = engine().cpu_id()] {
        ...
        pclass.ops++;
        pclass.nr_queued++;
        unsigned iops_weight = io_queue::read_request_base_count;
        size_t size = len;
        if (req_type == io_queue::request_type::write) {
            iops_weight = unsigned(queue._config.disk_req_write_to_read_ratio * io_queue::read_request_base_count);
            size = unsigned(queue._config.disk_bytes_write_to_read_ratio * len);
        }
        auto desc = std::make_unique<io_desc>(fairness_weight, iops_weight, size);
...
#endif
        ("io-properties-file", bpo::value<std::string>(), "path to a YAML file describing the chraracteristics of the disk")
...
public:
    ...
    void parse_config(boost::program_options::variables_map& configuration) {
        ...
        if (configuration.count("io-properties-file")) {
            YAML::Node doc = YAML::LoadFile(configuration["io-properties-file"].as<std::string>());
            auto disks = doc.as<std::vector<mountpoint_params>>();
            if (disks.size() != 1) {
                throw std::runtime_error(fmt::format("Currently only one mountpoint is supported. Specified {}", disks.size()));
            }
