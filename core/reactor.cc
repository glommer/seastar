...
reactor::submit_io_read(size_t len, Func prepare_io) {
    ++_aio_reads;
    _aio_read_bytes += len;
    return _io_queue->queue_request(len, [prepare_io = std::move(prepare_io)] {
...
template <typename Func>
std::result_of_t<Func()> io_queue::queue_request(size_t len, Func do_io) {
...
void smp::configure(boost::program_options::variables_map configuration)
{
    ...
    auto assign_io_queue = [io_info] (unsigned shard) {
        ...
        for (auto& coordinator: io_info.coordinators) {
            ...
            if (shard == cid) {
                ...
                return all_io_queues[vec_idx];
            } else {
                // We don't need no optimization,
                // We don't need no code to go,
                // Other threads could change this value,
                // Hey, compiler, leave the code alone!
                while (all_io_queues[vec_idx] == nullptr) { asm volatile("" ::: "memory"); }
                return all_io_queues[vec_idx];
            }
        }
        assert(0); // Impossible
    ...
    for (i = 1; i < smp::count; i++) {
        ...
        _threads.emplace_back([configuration, hugepages_path, i, allocation, io_info, assign_io_queue] {
            ...
            allocate_reactor();
            engine()._id = i;
            engine()._io_queue = assign_io_queue(i);
            start_all_queues();
            inited.wait();
    ...
    allocate_reactor();
    engine()._io_queue = assign_io_queue(0);
...
future<> check_direct_io_support(sstring path) {
    struct w {
        ...
        w(sstring path, open_flags flags, std::function<future<>()> cleanup) : path(std::move(path))
