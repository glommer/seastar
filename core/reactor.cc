...
reactor::handle_aio_error(::iocb* iocb, int ec) {
    switch (ec) {
        ...
        case EBADF: {
            ...
            delete pr;
            my_io_queue->requests_finished(1);
...
bool reactor::process_io()
{
    ...
    my_io_queue->requests_finished(n - nr_retry);
    return n;
}
...
io_queue::queue_request(shard_id coordinator, const io_priority_class& pc, size_t len, Func prepare_io) {
    ...
    return smp::submit_to(coordinator, [start, &pc, len, prepare_io = std::move(prepare_io), owner = engine().cpu_id()] {
        ...
        queue._fq.queue(pclass.ptr, weight, [&pclass, start, prepare_io = std::move(prepare_io), pr = pr.get()] () mutable {
            pclass.nr_queued--;
            pclass.queue_time = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::steady_clock::now() - start);
            engine().submit_io(pr, std::move(prepare_io));
        });
        pr.release();
        return fut;
    });
...
public:
    ...
    virtual bool try_enter_interrupt_mode() override {
        ...
        return _r.my_io_queue->in_flight() > 0 || _r._aio_eventfd;
    }
