...
const io_priority_class& default_priority_class() {
    static thread_local auto shard_default_class = [] {
        return engine().register_one_priority_class("default_priority", 1);
    }();
...
io_queue::priority_class_data& io_queue::find_or_create_class(const io_priority_class& pc) {
    ...
    if (it_pclass == _priority_classes.end()) {
        ...
        auto ret = _priority_classes.emplace(pc, make_lw_shared<priority_class_data>(name, _fq.register_priority_class(shares)));
        it_pclass = ret.first;
    }
...
io_queue::queue_request(shard_id coordinator, const io_priority_class& pc, size_t len, Func prepare_io) {
    return smp::submit_to(coordinator, [&pc, len, prepare_io = std::move(prepare_io)] {
        ...
        auto& pclass = queue.find_or_create_class(pc);
        pclass.bytes += len;
        pclass.ops++;
