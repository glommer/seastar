...
future<io_priority_class> reactor::create_io_priority_class(sstring name, uint32_t shares) {
    ...
    return smp::invoke_on_all([this, new_class, name = std::move(name), shares] () mutable {
        return parallel_for_each(engine()._io_queues | boost::adaptors::map_values, [new_class, name = std::move(name), shares] (io_queue& queue) {
            queue.register_one_priority_class(new_class, name, shares);
            return make_ready_future<>();
        });
    }).then([new_class] {
...
void io_queue::register_one_priority_class(io_priority_class iop, sstring name, uint32_t shares) {
...
void smp::configure(boost::program_options::variables_map configuration)
{
    ...
    struct io_queue_params {
        io_queue::config cfg;
        fair_queue* fq_ptr;
    };

    std::unordered_map<dev_t, std::vector<io_queue_params>> all_io_queues;
        ...
        for (auto& coordinator: io_info.coordinators) {
            ...
            if (shard == cid) {
                for (auto& id : disk_config.device_ids()) {
                    ...
                    all_io_queues[id][vec_idx] = io_queue_params{std::move(cfg), new fair_queue(io_queue::make_fair_queue_config(cfg))};
                }
            }
            return vec_idx;
        }
