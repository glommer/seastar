...
public:
    ...
    // before the syscall thread loads this value, so we'll need full seq_cst (the default).
    void enter_interrupt_mode() { _main_thread_idle.store(true); }
    void exit_interrupt_mode() { _main_thread_idle.store(false); }
