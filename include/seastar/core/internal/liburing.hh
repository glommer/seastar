...
private:
    ...
    chunked_fifo<noncopyable_function<void(io_uring_sqe&)>> _pending_sqe;

    ring_completion_method _ring_type;
    std::function<void(io_uring_cqe*)> _process_cqe;
    ...
    uring_context(std::function<void(io_uring_cqe*)> process_cqe, ring_completion_method rtype, unsigned cq_entries);
...
public:
    ...
    void push_prio_sqe(noncopyable_function<void(io_uring_sqe&)> func);

    // Pushes a sqe to the userspace submission queue. It will be submitted to the
    // kernel at a later time
    void push_sqe(noncopyable_function<void(io_uring_sqe&)> fun);
