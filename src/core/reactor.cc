...
void
reactor::submit_io(io_request req) {
...
aio_storage_context::submit_work() {
    ...
    while ((pending > to_submit) && _iocb_pool.has_capacity()) {
        ...
        submission_queue.push_back(&io);
    }

    size_t submitted = 0;
    while (to_submit > submitted) {
        auto nr = to_submit - submitted;
        auto iocbs = submission_queue.data() + submitted;
...
reactor::fdatasync(int fd) {
    ...
    if (_have_aio_fsync) {
        try {
            ...
            req.attach_kernel_completion(desc.release());
...
public:
    ...
    virtual bool poll() final override {
        return _r.reap_kernel_events_completions();
    }
    virtual bool pure_poll() override final {
        return poll(); // actually performs work, but triggers no user continuations, so okay
    }
...
int reactor::run() {
    ...
    poller kernel_events_final_completion_poller(std::make_unique<kernel_events_completion_pollfn>(*this));
