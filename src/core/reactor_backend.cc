...
future<> reactor_backend_aio::poll(pollable_fd_state& fd, promise_io_desc* desc, int events) {
    if (!_r->_epoll_poller) {
    ...
    try {
        ...
        *desc = promise_io_desc{};
        set_user_data(*iocb, desc);
        _polling_io.queue(iocb);
        return desc->get_future().discard_result();
    } catch (...) {
...
future<> reactor_backend_epoll::get_epoll_future(pollable_fd_state& pfd,
        promise_io_desc *desc, int event) {
