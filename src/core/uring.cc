...
void uring_context::prepare_sqe(const internal::io_request& req, io_uring_sqe& sqe) {
    ...
    // It is usually better to do operations inline so they are ready
    // in the next reap cycle if they are ready.
    // Some operations, like storage I/O, for instance, are always at
    // a risk of sleeping and we want to force those to be async. The
    // submitter is responsible for telling us what to do.
    if (req.should_force_async()) {
        ::io_uring_sqe_set_flags(&sqe, IOSQE_ASYNC);
    }
}

unsigned uring_context::flush() {
    auto account = defer([this, start = std::chrono::steady_clock::now()] {
        auto end = std::chrono::steady_clock::now();
        _flushes++;
        _flush_time += end - start;
    });
